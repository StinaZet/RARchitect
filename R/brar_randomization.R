...
brar_randomization <- function(
  ...
  # Initialize alpha/beta based on priors and observed outcomes
  ...
  for (b in batches) {
    ...
    if (is.character(clipping) && clipping == "adaptive") {
      adaptive_block <- b
      clipping_val <- min(1/arms, (1/arms) * adaptive_block^(-0.7))
      clipping_val <- max(clipping_val, 1e-6)
    } else if (is.numeric(clipping) && clipping > 0) {
      clipping_val <- clipping
    } else {
      clipping_val <- 0
    }
