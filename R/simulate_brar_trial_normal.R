                                        ...
                                        known_var = FALSE)
{
  ...
  for (i in start_block_idx:Nblocks) {
    ...
    if(randmethod == "block")
    {
      ...
      selected_arm[idx] = sample(arm_assignments, size = current_block_size, replace = FALSE)
      ...
      outcomes_block = numeric(current_block_size)
      ...
      for (iii in 1:current_block_size)
      {
        ...
        outcome = stats::rnorm(1, mean = true_means[draw], sd = true_sds[draw])
        outcomes_block[iii] = outcome
      ...
      # --- 6. Save results ---
      allocation_probs_matrix[idx, ] = probs_per_draw
      selected_arm[idx] = selected_arm_block
      outcomes[idx] = outcomes_block
    ...
    # --- Simulate Outcomes (Only needed for "block" and "coin" as "urn" already simulated them) ---
    if (randmethod != "urn") {
