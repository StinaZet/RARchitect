...
test_that("posterior_bin_sim returns valid probabilities", {
  set.seed(1)
  ap <- posterior_bin_sim(M = 5000, alphas = c(5,5), betas = c(5,5))
...
test_that("posterior_bin_exact symmetric case gives 0.5", {
  ap <- posterior_bin_exact(c(5,5), c(5,5))
  expect_equal(ap, c(0.5, 0.5))
})

test_that("posterior_bin_sim favors better arm", {
  set.seed(2)
  ap <- posterior_bin_sim(M = 5000, alphas = c(20,5), betas = c(5,20))
  expect_gt(ap[1], ap[2])
})
...
test_that("posterior_norm_exact symmetric case", {
  ap <- posterior_norm_exact(c(0,0), c(1,1))
  expect_equal(ap, c(0.5, 0.5))
})

test_that("posterior_norm_sim returns valid probabilities", {
  set.seed(1)
  ap <- posterior_norm_sim(5000, means = c(0,1), sds = c(1,1))
...
test_that("posterior_exp_exact symmetric case", {
  ap <- posterior_exp_exact(c(5,5), c(2,2))
  expect_equal(ap, c(0.5, 0.5))
})

test_that("posterior_exp_sim favors larger mean", {
  set.seed(1)
  ap <- posterior_exp_sim(5000, shapes = c(10,2), rates = c(1,1))
  expect_gt(ap[1], ap[2])
})
